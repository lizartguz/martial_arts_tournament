...
<head>
    ...
    <title>{{ __('messages.errors.419.title') }} | {{ \App\Support\AdminPanel::brandName() }}</title>

    <link rel="icon" type="image/png" href="{{ \App\Support\AdminPanel::favicon() }}?v=20260510">
...
<body class="min-h-screen bg-slate-50 text-slate-950 antialiased">
    <main class="flex min-h-screen items-center justify-center px-6 py-12">
        <section class="w-full max-w-xl text-center">
            <img
                src="{{ \App\Support\AdminPanel::brandLogo() }}"
                alt="{{ \App\Support\AdminPanel::brandName() }}"
                class="mx-auto mb-8 h-16 w-auto max-w-[220px] object-contain shadow-sm"
            >

            <div class="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-semibold text-amber-800">
                <i class="fas fa-clock" aria-hidden="true"></i>
                <span>{{ __('messages.errors.419.eyebrow') }}</span>
            </div>

            <h1 class="text-3xl font-bold tracking-normal text-slate-950 sm:text-4xl">
                {{ __('messages.errors.419.title') }}
            </h1>
            ...
            <div class="mt-8">
                ...
                >
                    ...
                    <span>{{ __('messages.errors.419.login') }}</span>
                </a>
            </div>
        </section>
    </main>
