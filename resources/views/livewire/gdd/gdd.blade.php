...
<div x-data="{ selectedRange: @entangle('selectedRange') }">
    <h5 class="mb-4 w-full text-center text-base font-semibold text-gray-800 dark:text-gray-200">{{ __('messages.gdd.page') }}</h5>
    ...
    <div x-cloak>
        ...
        <div class="mb-4 border-b border-gray-200 pb-4 dark:border-gray-700">
            <div class="flex w-full flex-col gap-4 md:flex-row">
                <div class="w-full" wire:ignore>
                    <label class="tw-label">{{ __('messages.gdd.station_label') }}</label>
                    <x-adminlte-select-bs
                        id="station-select"
                        name="station-select"
                        title="{{ __('messages.gdd.station_placeholder') }}"
                ...
                <div class="w-full md:max-w-36">
                    <label class="tw-label">{{ __('messages.gdd.range_label') }}</label>
                ...
                <div class="w-full" id="pnlDate" x-show="selectedRange == null || selectedRange == 'daily'">
                    <label class="tw-label">{{ __('messages.gdd.date_label') }}</label>
                ...
                <div class="flex w-full gap-3 md:max-w-64" id="pnlYear" x-show="selectedRange != 'daily' && selectedRange != null">
                    <div class="w-full">
                        <label class="tw-label">{{ __('messages.gdd.year_date_label') }}</label>
                    ...
                    <div class="w-full" id="pnlMonth" x-show="selectedRange == 'monthly'">
                        <label class="tw-label">{{ __('messages.gdd.month_date_label') }}</label>
                ...
                <div class="flex justify-between gap-3">
                    <div class="min-w-24" wire:ignore>
                        <label class="tw-label">{{ __('messages.gdd.base_label') }}</label>
                        <select class="tw-input" id="base-select" name="base-select" title="{{ __('messages.gdd.base_placeholder') }}">
                            @foreach ($baseList as $key => $item)
                                <option value="{{ $key }}" {{ $selectedBase == $key ? 'selected' : '' }}>{{ $item }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="flex items-end">
                        <div class="flex w-36 items-center gap-2">
                            <button class="tw-btn tw-btn-emerald w-full" wire:loading.attr="disabled" onclick="checkParams()">
                                <span wire:loading.remove>{{ __('messages.gdd.show_button_label') }}</span>
                                <span wire:loading>{{ __('messages.gdd.processing_button_label') }}</span>
                            </button>
        ...
        @if ($showResults)
            <div class="overflow-x-auto" id="pnlResults" name="pnlResults">
                ...
                @else
                    <div class="py-6 text-center text-gray-500 dark:text-gray-400">{{ __('messages.gdd.result_no_data') }}</div>
                @endif
            </div>
                ...
                <div id="pnlFooter" class="mt-4 flex flex-col gap-3">
                    <div class="flex items-center">
                        <span class="border-2 border-gray-800 bg-white px-3 py-1 text-sm font-medium dark:border-gray-400 dark:bg-gray-700 dark:text-gray-200">
                            {{ __('messages.gdd.total_label') }} = {{ $totalGDD }}
                        </span>
                    </div>
                    <div>{!! $data->links() !!}</div>
                </div>
...
<script>
    ...
    Livewire.on('showData', ([data]) => {
        ...
        ['range-select','from-date-input','to-date-input','year-select','month-select','base-select']
    ...
    function handleBlurFromDate() {
        ...
        let d = new Date(val);
        if (!isValidDate(val)) { Swal.fire({ text: @js(__('messages.gdd.errors.select_from_date')) }); return; }
        if (d > maxDate) { Swal.fire({ text: @js(__('messages.gdd.errors.select_from_date_greater_than', ['maxdate' => $this->maxDate->format('d/m/Y')])) }); return; }
        if (d < minDate) { Swal.fire({ text: @js(__('messages.gdd.errors.select_from_date_less_than', ['mindate' => $this->minDate->format('d/m/Y')])) }); return; }
    }
    ...
    function handleBlurToDate() {
        ...
        if (!isValidDate(toVal)) { Swal.fire({ text: @js(__('messages.gdd.errors.select_to_date')) }); return; }
        if (toDate > maxDate) { Swal.fire({ text: @js(__('messages.gdd.errors.select_to_date_greater_than', ['maxdate' => $this->maxDate->format('d/m/Y')])) }); return; }
        if (toDate < minDate) { Swal.fire({ text: @js(__('messages.gdd.errors.select_to_date_less_than', ['mindate' => $this->minDate->format('d/m/Y')])) }); return; }
        if (isValidDate(fromVal)) {
            if (toDate < fromDate) { Swal.fire({ text: @js(__('messages.gdd.errors.select_to_date_less_than_from_date')) }); return; }
            if (fromDate.getUTCFullYear() != toDate.getUTCFullYear()) { Swal.fire({ text: @js(__('messages.gdd.errors.select_year_to_date_equal_than_year_from_date')) }); return; }
        }
    }
    ...
    function checkParams() {
        ...
        if (station < 1) { Swal.fire({ text: @js(__('messages.gdd.errors.select_station')) }); return; }
        if (range == 'daily') {
            if (!isValidDate(fromVal)) { Swal.fire({ text: @js(__('messages.gdd.errors.select_from_date')) }); return; }
            if (fromDate > maxDate) { Swal.fire({ text: @js(__('messages.gdd.errors.select_from_date_greater_than', ['maxdate' => $this->maxDate->format('d/m/Y')])) }); return; }
            if (fromDate < minDate) { Swal.fire({ text: @js(__('messages.gdd.errors.select_from_date_less_than', ['mindate' => $this->minDate->format('d/m/Y')])) }); return; }
            if (!isValidDate(toVal))  { Swal.fire({ text: @js(__('messages.gdd.errors.select_to_date')) }); return; }
            if (toDate > maxDate) { Swal.fire({ text: @js(__('messages.gdd.errors.select_to_date_greater_than', ['maxdate' => $this->maxDate->format('d/m/Y')])) }); return; }
            if (toDate < minDate) { Swal.fire({ text: @js(__('messages.gdd.errors.select_to_date_less_than', ['mindate' => $this->minDate->format('d/m/Y')])) }); return; }
            if (isValidDate(fromVal)) {
        ...
        Livewire.dispatch('processParams', {
            ...
            selectedYear: year, selectedMonth: month, selectedBase: base,
        });
    }
</script>
@stop
