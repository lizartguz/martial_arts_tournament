...
@section('content')
    ...
    <section class="rounded-lg border border-white/10 bg-white/5 p-4">
        <div class="grid gap-4 md:grid-cols-3">
            <div>
                ...
                <p class="mt-1 font-semibold">{{ $purchaseRequest->event?->name ?? $purchaseRequest->plan?->name ?? __('mma.admin.purchase_requests.request_types.'.$purchaseRequest->request_type) }}</p>
            </div>
            <div>
                <p class="text-sm text-gray-400">{{ __('mma.subscriber_portal.columns.channel') }}</p>
                <p class="mt-1 font-semibold">{{ __('mma.admin.purchase_requests.channels.'.$purchaseRequest->preferred_channel) }}</p>
            </div>
            <div>
                <p class="text-sm text-gray-400">{{ __('mma.subscriber_portal.columns.status') }}</p>
                <p class="mt-1 font-semibold">{{ __('mma.admin.purchase_requests.status.'.($requestStatus[$purchaseRequest->status] ?? 'pending')) }}</p>
            </div>
            <div>
                <p class="text-sm text-gray-400">{{ __('mma.subscriber_portal.columns.date') }}</p>
                <p class="mt-1 font-semibold">{{ $purchaseRequest->created_at?->format('d/m/Y H:i') }}</p>
            </div>
            <div>
                <p class="text-sm text-gray-400">{{ __('mma.subscriber_portal.purchases.proof_status') }}</p>
                <p class="mt-1 font-semibold">
                    {{ filled($purchaseRequest->payment_proof_path) ? __('mma.subscriber_portal.purchases.proof_uploaded_label') : __('mma.subscriber_portal.purchases.proof_missing_label') }}
                </p>
            </div>
            @if ($purchaseRequest->event)
                <div>
                    <p class="text-sm text-gray-400">{{ __('mma.subscriber_portal.purchases.related_event') }}</p>
                    <p class="mt-1 font-semibold">
                        @if ((int) $purchaseRequest->event->status === 1)
                            <a href="{{ route('landing.events.show', $purchaseRequest->event->slug) }}" target="_blank" rel="noopener" class="text-emerald-400 hover:underline">{{ $purchaseRequest->event->name }}</a>
                        @else
                            {{ $purchaseRequest->event->name }}
                        @endif
                    </p>
                </div>
            @endif
        ...
        @if ($purchaseRequest->message)
            <div class="mt-4 border-t border-white/10 pt-4">
                <p class="text-sm text-gray-400">{{ __('mma.subscriber_portal.purchases.message') }}</p>
                <p class="mt-1 text-sm">{{ $purchaseRequest->message }}</p>
            </div>
        @endif
    </section>
    ...
    @if (in_array((int) $purchaseRequest->status, [0, 1], true))
        <section class="mt-5 rounded-lg border border-white/10 bg-white/5 p-4">
            ...
            <form method="POST" action="{{ route('subscriber.purchases.requests.proof', $purchaseRequest) }}" enctype="multipart/form-data" class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end">
                @csrf
                <div class="flex-1">
