<div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
    <div class="tw-panel-header">
        <div>
            <strong class="text-gray-800 dark:text-gray-200">{{ __('mma.admin.fighter_media.table_title') }}</strong>
            <div class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{{ __('mma.admin.fighter_media.table_subtitle') }}</div>
        </div>
        @can('fighter_media.create')
            <button type="button" class="tw-btn tw-btn-emerald" wire:click="create">
                <i class="fas fa-plus text-xs"></i>{{ __('mma.admin.fighter_media.create') }}
            </button>
        @endcan
    </div>

    <div class="mb-4 grid gap-3 xl:grid-cols-[minmax(220px,1fr)_220px_140px_130px_110px_auto]">
        <div>
            ...
            <input type="text" class="tw-input-sm" wire:model.live.debounce.400ms="search"
                   placeholder="{{ __('mma.admin.fighter_media.search_placeholder') }}">
        </div>
        <div>
            <label class="tw-label">{{ __('mma.admin.fighter_media.filters.fighter') }}</label>
            <select class="tw-input-sm" wire:model.live="fighterId">
        ...
        <div>
            <label class="tw-label">{{ __('mma.admin.fighter_media.filters.file_type') }}</label>
    ...
    <div class="tw-table-wrap">
        <table class="tw-table">
            <thead class="themed-thead">
                <tr>
                    <th>{{ __('mma.admin.fighter_media.columns.media') }}</th>
                    <th>{{ __('mma.admin.fighter_media.columns.fighter') }}</th>
                    <th>{{ __('mma.admin.fighter_media.columns.order') }}</th>
                    <th>{{ __('mma.admin.fighter_media.columns.status') }}</th>
                    <th class="text-center">{{ __('mma.admin.common.columns.actions') }}</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($mediaItems as $media)
                    <tr wire:key="fighter-media-{{ $media->id }}">
                        <td>
                            <div class="flex min-w-[280px] items-center gap-3">
                                ...
                                <div>
                                    <div class="font-semibold text-gray-900 dark:text-gray-100">{{ $media->title ?? __('mma.admin.fighter_media.untitled') }}</div>
                                    <div class="text-xs text-gray-500">{{ $this->fileTypeLabel($media->file_type) }}</div>
                                    @if ($media->is_featured)
                                        <span class="tw-badge tw-badge-blue mt-1">{{ __('mma.admin.fighter_media.featured') }}</span>
                                    @endif
                                </div>
                            </div>
                        </td>
                        <td>
                            <div>{{ $this->fighterName($media->fighter) }}</div>
                            <div class="text-xs text-gray-500">{{ $media->fighter?->nickname ?? __('mma.admin.common.not_available') }}</div>
                        </td>
                        <td>{{ $media->display_order }}</td>
                        <td>
                            <span class="tw-badge {{ (int) $media->status === 1 ? 'tw-badge-green' : 'tw-badge-gray' }}">
                        ...
                        <td>
                            <div class="flex justify-center" x-data="{ open: false }" @click.outside="open = false">
                                ...
                                <div x-show="open" x-cloak
                                     class="absolute z-20 mt-8 w-40 rounded border border-gray-200 bg-white py-1 shadow-lg dark:border-gray-700 dark:bg-gray-800">
                                    @can('fighter_media.update')
    ...
    @if ($showModal)
        <x-tw-modal close="closeModal" max-width="5xl" :title="$editingId ? __('mma.admin.fighter_media.edit') : __('mma.admin.fighter_media.create')" icon="fas fa-photo-video">
            <div class="grid gap-4 md:grid-cols-2">
                <div class="md:col-span-2">
                    <label class="tw-label">{{ __('mma.admin.fighter_media.form.fighter_id') }}</label>
                ...
                <div>
                    <label class="tw-label">{{ __('mma.admin.fighter_media.form.file_type') }}</label>
                ...
                <div>
                    <label class="tw-label">{{ __('mma.admin.fighter_media.form.display_order') }}</label>
                ...
                <div class="md:col-span-2">
                    <label class="tw-label">{{ __('mma.admin.fighter_media.form.title') }}</label>
                ...
                @if ($form['file_type'] === 'image')
                    <div class="md:col-span-2">
                        <label class="tw-label">{{ __('mma.admin.fighter_media.form.media_image') }}</label>
                ...
                @else
                    <div class="md:col-span-2">
                        <label class="tw-label">{{ __('mma.admin.fighter_media.form.file_path') }}</label>
                ...
                @endif
                <div>
                    <label class="tw-label">{{ __('mma.admin.fighter_media.form.status') }}</label>
                ...
                <div class="flex items-end">
                    <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
                        ...
                        <span>{{ __('mma.admin.fighter_media.form.is_featured') }}</span>
                    </label>
                </div>
                <div class="md:col-span-2">
                    <label class="tw-label">{{ __('mma.admin.fighter_media.form.description') }}</label>
    ...
    @if ($showDeleteModal)
        <x-tw-modal close="closeDeleteModal" max-width="md" :title="__('mma.admin.fighter_media.delete_title')" icon="fas fa-trash" header-class="bg-red-600 text-white">
            <p class="text-sm text-gray-700 dark:text-gray-300">
                {{ __('mma.admin.fighter_media.delete_warning') }} <strong>{{ $deleteName }}</strong>
            </p>
            <x-slot:footer>
